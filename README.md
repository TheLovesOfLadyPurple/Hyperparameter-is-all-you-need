...
This repo implement a 8-step inference algorithm which FID performance is better than the results from the normal DPM++2m solver with 20-step inference. Meanwhile, it also support 5 and 6 step inference which generate result comparable with the latest diffussion distillation algorithm.  Last but not least, it's the first solver sampling an 1024 x 1024 image within 8-step without additional training, and its' FID is comparable than the SOTA distillation algorithm.
...
You can also use the counterfeit v3.0 to generate acgn image.  The result is in ./gen_img_val_v15. The model is in: https://civitai.com/models/4468/counterfeit-v30
